...
class Assets
{
    ...
    public function overrideLocalizeScript($context)
    {
        ...
        if ($this->isScreenBase('upload')) {
            $mlNotices = ['showLicenseNotice' => !$isLicenseActivated && !$isLicenseNoticeDismissed && current_user_can('install_plugins'), 'showTaxImportNotice' => !\MatthiasWeb\RealMediaLibrary\comp\ExImport::getInstance()->isImportTaxNoticeDismissed() && (\count(\MatthiasWeb\RealMediaLibrary\comp\ExImport::getInstance()->getHierarchicalTaxos()) > 0 || \MatthiasWeb\RealMediaLibrary\comp\ExImport::getInstance()->hasMediaLibraryFolders())];
        } else {
